...
kernels::GemmVersion parse_gemm_version(const char* version) {
    ...
    if (name == "naive") return kernels::GemmVersion::Naive;
    if (name == "tiled") return kernels::GemmVersion::Tiled;
    if (name == "double_buffer") return kernels::GemmVersion::DoubleBuffer;
