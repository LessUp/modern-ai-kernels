...
public:
    void SetUp(const benchmark::State& state) override {
        ...
        TC_CUDA_CHECK(cudaMemcpy(d_input, h_input.data(), N * C * H * W * sizeof(float),
                                 cudaMemcpyHostToDevice));
        TC_CUDA_CHECK(cudaMemcpy(d_weight, h_weight.data(), K * C * R * S * sizeof(float),
                                 cudaMemcpyHostToDevice));
    }
...
BENCHMARK_DEFINE_F(Conv2DBenchmark, Naive)(benchmark::State& state) {
    for (auto _ : state) {
        launch_conv2d_naive(d_input, d_weight, static_cast<const float*>(nullptr), d_output, N, C,
                            H, W, K, R, S, stride, stride, padding, padding);
...
public:
    void SetUp(const benchmark::State& state) override {
        ...
        TC_CUDA_CHECK(cudaMemcpy(d_input, h_input.data(), N * C * H * W * sizeof(float),
                                 cudaMemcpyHostToDevice));
        TC_CUDA_CHECK(cudaMemcpy(d_weight, h_weight.data(), C * R * S * sizeof(float),
                                 cudaMemcpyHostToDevice));
    }
...
BENCHMARK_DEFINE_F(DepthwiseConv2DBenchmark, Depthwise)(benchmark::State& state) {
    for (auto _ : state) {
        launch_conv2d_depthwise(d_input, d_weight, static_cast<const float*>(nullptr), d_output, N,
                                C, H, W, R, S, stride, stride, padding, padding);
        cudaDeviceSynchronize();
    }
