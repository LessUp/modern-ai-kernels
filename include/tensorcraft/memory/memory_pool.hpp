...
public:
    ...
    // Move only
    PoolPtr(PoolPtr&& other) noexcept : ptr_(other.ptr_), count_(other.count_) {
        other.ptr_ = nullptr;
        other.count_ = 0;
    }
...
public:
    ...
    explicit PinnedPtr(size_t count)
        : ptr_(static_cast<T*>(PinnedMemory::allocate(count * sizeof(T)))), count_(count) {}

    ~PinnedPtr() { PinnedMemory::deallocate(ptr_); }
