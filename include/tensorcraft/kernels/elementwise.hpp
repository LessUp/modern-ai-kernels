...
__global__ void elementwise_kernel(const T* TC_RESTRICT input, T* TC_RESTRICT output, size_t n,
                                   Func func) {
...
__global__ void elementwise_kernel_naive(const T* TC_RESTRICT input, T* TC_RESTRICT output,
                                         size_t n, Func func) {
 ...
 */
template <typename T, typename Func, int VecSize = 4>
__global__ void elementwise_binary_kernel(const T* TC_RESTRICT input1, const T* TC_RESTRICT input2,
                                          T* TC_RESTRICT output, size_t n, Func func) {
    using VecT = AlignedVector<T, VecSize>;
 ...
 */
template <typename T, typename Func>
__global__ void elementwise_binary_kernel_naive(const T* TC_RESTRICT input1,
                                                const T* TC_RESTRICT input2, T* TC_RESTRICT output,
                                                size_t n, Func func) {
    const size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    const size_t stride = blockDim.x * gridDim.x;
...
struct LeakyReLU {
    ...
    TC_DEVICE_INLINE T operator()(T x) const { return x > T(0) ? x : alpha * x; }
};
...
void launch_elementwise(const T* input, T* output, size_t n, Func func,
    ...
    if (use_vectorized && is_aligned<T, vec_size>(input) && is_aligned<T, vec_size>(output)) {
        ...
        elementwise_kernel<T, Func, vec_size>
            <<<grid_size, block_size, 0, stream>>>(input, output, n, func);
    } else {
        int grid_size = static_cast<int>((n + block_size - 1) / block_size);
        grid_size = std::min(grid_size, 65535);

        elementwise_kernel_naive<T, Func>
            <<<grid_size, block_size, 0, stream>>>(input, output, n, func);
    }
...
void launch_elementwise_binary(const T* input1, const T* input2, T* output, size_t n, Func func,
    ...
    if (aligned) {
        ...
        elementwise_binary_kernel_naive<T, Func>
            <<<grid_size, block_size, 0, stream>>>(input1, input2, output, n, func);
    }
