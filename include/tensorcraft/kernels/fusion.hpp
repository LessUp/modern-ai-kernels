...
struct EpilogueBias {
    ...
    TC_DEVICE_INLINE T operator()(T val, int row, int col) const { return val + bias[col]; }
};
...
__global__ void gemm_fused_kernel(const T* TC_RESTRICT A, const T* TC_RESTRICT B, T* TC_RESTRICT C,
                                  int M, int N, int K, T alpha, Epilogue epilogue) {
...
template <typename T>
__global__ void quantize_int8_kernel(const T* TC_RESTRICT input, int8_t* TC_RESTRICT output,
...
template <typename T>
__global__ void dequantize_int8_kernel(const int8_t* TC_RESTRICT input, T* TC_RESTRICT output,
                                       float scale, int zero_point, size_t n) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n)
        return;
...
template <typename T>
__global__ void compute_quant_params_kernel(const T* TC_RESTRICT input, float* TC_RESTRICT min_val,
                                            float* TC_RESTRICT max_val, size_t n) {
...
template <typename T>
__global__ void quantize_fp8_e4m3_kernel(const T* TC_RESTRICT input,
                                         __nv_fp8_e4m3* TC_RESTRICT output, float scale, size_t n) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n)
        return;
...
template <typename T>
__global__ void dequantize_fp8_e4m3_kernel(const __nv_fp8_e4m3* TC_RESTRICT input,
                                           T* TC_RESTRICT output, float scale, size_t n) {
    size_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n)
        return;
...
void launch_gemm_fused(const T* A, const T* B, T* C, int M, int N, int K, T alpha,
                       Epilogue epilogue, cudaStream_t stream = nullptr) {
    if (M == 0 || N == 0 || K == 0)
        return;
    ...
    gemm_fused_kernel<T, Epilogue, TILE_SIZE>
        <<<grid, block, 0, stream>>>(A, B, C, M, N, K, alpha, epilogue);
    TC_CUDA_CHECK_LAST();
}

template <typename T>
void launch_quantize_int8(const T* input, int8_t* output, float scale, int zero_point, size_t n,
    ...
    quantize_int8_kernel<T>
        <<<grid_size, BLOCK_SIZE, 0, stream>>>(input, output, scale, zero_point, n);
    TC_CUDA_CHECK_LAST();
}

template <typename T>
void launch_dequantize_int8(const int8_t* input, T* output, float scale, int zero_point, size_t n,
                            cudaStream_t stream = nullptr) {
    if (n == 0)
        return;
...
template <typename T>
void gemm_bias_gelu(const T* A, const T* B, const T* bias, T* C, size_t M, size_t N, size_t K,
                    cudaStream_t stream = nullptr) {
...
template <typename T>
void gemm_bias_relu(const T* A, const T* B, const T* bias, T* C, size_t M, size_t N, size_t K,
                    cudaStream_t stream = nullptr) {
