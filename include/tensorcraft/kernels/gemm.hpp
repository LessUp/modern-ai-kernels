 ...
 */
template <typename T>
__global__ void gemm_naive_kernel(const T* TC_RESTRICT A, const T* TC_RESTRICT B, T* TC_RESTRICT C,
...
__global__ void gemm_tiled_kernel(const T* TC_RESTRICT A, const T* TC_RESTRICT B, T* TC_RESTRICT C,
                                  int M, int N, int K, T alpha, T beta) {
 ...
 */
template <typename T, int TILE_SIZE = 32>
__global__ void gemm_double_buffer_kernel(const T* TC_RESTRICT A, const T* TC_RESTRICT B,
                                          T* TC_RESTRICT C, int M, int N, int K, T alpha, T beta) {
...
__global__ void gemm_wmma_kernel(const half* TC_RESTRICT A, const half* TC_RESTRICT B,
                                 float* TC_RESTRICT C, int M, int N, int K, float alpha,
                                 float beta) {
    ...
    if (beta != 0.0f) {
        ...
        if (warp_row_offset < M && warp_col_offset < N) {
            load_matrix_sync(c_old_frag, C + warp_row_offset * N + warp_col_offset, N,
                             mem_row_major);
            for (int i = 0; i < c_frag.num_elements; ++i) {
 ...
 */
template <typename T>
__global__ void transpose_naive_kernel(const T* TC_RESTRICT input, T* TC_RESTRICT output, int rows,
                                       int cols) {
    const int row = blockIdx.y * blockDim.y + threadIdx.y;
    const int col = blockIdx.x * blockDim.x + threadIdx.x;
...
void launch_gemm(const T* A, const T* B, T* C, int M, int N, int K, T alpha = T(1), T beta = T(0),
                 GemmVersion version = GemmVersion::Tiled, cudaStream_t stream = nullptr) {
    if (M == 0 || N == 0 || K == 0)
        return;
    ...
    switch (version) {
        case GemmVersion::Naive:
            gemm_naive_kernel<T><<<grid, block, 0, stream>>>(A, B, C, M, N, K, alpha, beta);
            break;
        ...
        case GemmVersion::DoubleBuffer:
            gemm_double_buffer_kernel<T, TILE_SIZE>
                <<<grid, block, 0, stream>>>(A, B, C, M, N, K, alpha, beta);
            break;
 ...
 */
inline void launch_gemm_wmma(const half* A, const half* B, float* C, int M, int N, int K,
    ...
    gemm_wmma_kernel<WMMA_M, WMMA_N, 16><<<grid, block, 0, stream>>>(A, B, C, M, N, K, alpha, beta);
 ...
 */
template <typename T>
void launch_transpose(const T* input, T* output, int rows, int cols, bool use_shared = true,
    ...
    if (use_shared) {
        transpose_shared_kernel<T, TILE_SIZE>
            <<<grid, block, 0, stream>>>(input, output, rows, cols);
    } else {
        transpose_naive_kernel<T><<<grid, block, 0, stream>>>(input, output, rows, cols);
    }
...
template <typename T>
void gemm(const T* A, const T* B, T* C, size_t M, size_t N, size_t K, T alpha = T(1), T beta = T(0),
          cudaStream_t stream = nullptr) {
    launch_gemm(A, B, C, static_cast<int>(M), static_cast<int>(N), static_cast<int>(K), alpha, beta,
                GemmVersion::Tiled, stream);
}

/// Matrix transpose
template <typename T>
void transpose(const T* input, T* output, size_t rows, size_t cols, cudaStream_t stream = nullptr) {
    launch_transpose(input, output, static_cast<int>(rows), static_cast<int>(cols), true, stream);
}
