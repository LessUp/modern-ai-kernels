...
__global__ void softmax_kernel(const T* TC_RESTRICT input, T* TC_RESTRICT output, int rows,
                               int cols) {
 ...
 */
template <typename T, int BLOCK_SIZE = 256>
__global__ void softmax_online_kernel(const T* TC_RESTRICT input, T* TC_RESTRICT output, int rows,
                                      int cols) {
    const int row = blockIdx.x;
    if (row >= rows)
        return;
 ...
 */
template <typename T>
void launch_softmax(const T* input, T* output, int rows, int cols, cudaStream_t stream = nullptr) {
    if (rows == 0 || cols == 0)
        return;
    ...
    softmax_online_kernel<T, BLOCK_SIZE>
        <<<rows, BLOCK_SIZE, 0, stream>>>(input, output, rows, cols);
...
void softmax(const T* input, T* output, size_t batch_size, size_t dim,
    ...
    launch_softmax(input, output, static_cast<int>(batch_size), static_cast<int>(dim), stream);
}
