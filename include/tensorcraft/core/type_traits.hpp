...
template <typename T>
struct is_floating
    : std::bool_constant<std::is_floating_point_v<T> || is_half_v<T> || is_fp8_v<T>> {};
...
template <typename T>
struct is_numeric
    : std::integral_constant<bool, std::is_arithmetic<T>::value || is_half_v<T> || is_fp8_v<T>> {};
 ...
 */
enum class DataType { FP32, FP16, BF16, FP8_E4M3, FP8_E5M2, INT8, INT32, INT64 };
