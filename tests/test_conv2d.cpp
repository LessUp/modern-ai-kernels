...
protected:
    ...
    std::vector<float> reference_conv2d(
...
TEST_F(Conv2DTest, NaiveCorrectness) {
    ...
    TC_CUDA_CHECK(cudaMemcpy(d_in, h_input.data(), N * C * H * W * sizeof(float), cudaMemcpyHostToDevice));
    TC_CUDA_CHECK(cudaMemcpy(d_w,  h_weight.data(), K * C * R * S * sizeof(float), cudaMemcpyHostToDevice));
    ...
    TC_CUDA_CHECK(cudaMemcpy(h_out.data(), d_out, h_out.size() * sizeof(float), cudaMemcpyDeviceToHost));
...
TEST_F(Conv2DTest, DepthwiseCorrectness) {
    ...
    TC_CUDA_CHECK(cudaMemcpy(d_in, h_input.data(), N * C * H * W * sizeof(float), cudaMemcpyHostToDevice));
    TC_CUDA_CHECK(cudaMemcpy(d_w,  h_weight.data(), C * R * S * sizeof(float), cudaMemcpyHostToDevice));

    conv2d_depthwise(d_in, d_w, static_cast<const float*>(nullptr), d_out,
                     N, C, H, W, R, S, stride, pad);
    TC_CUDA_CHECK(cudaDeviceSynchronize());
