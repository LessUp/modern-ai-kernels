...
void launch_batchnorm(const float* input,
                      ...
                      float eps,
                      bool training) {
    kernels::launch_batchnorm(input, gamma, beta, running_mean, running_var, output, n, c, h, w, eps, training);
}
...
void conv2d(const float* input,
            ...
            int stride,
            int pad) {
    kernels::conv2d(input, weight, bias, output, batch, channels, height, width, filters, kernel_h, kernel_w, stride, pad);
}
...
void conv2d_depthwise(const float* input,
                      ...
                      int stride,
                      int pad) {
    kernels::conv2d_depthwise(input, weight, bias, output, batch, channels, height, width, kernel_h, kernel_w, stride, pad);
}
...
void launch_flash_attention(const float* q,
                            ...
                            int head_dim,
                            float scale) {
    kernels::launch_flash_attention(q, k, v, output, batch_size, num_heads, seq_len, head_dim, scale);
}
...
void launch_rope(float* input,
                 ...
                 int head_dim,
                 int start_pos) {
    kernels::launch_rope(input, cos_table, sin_table, batch_size, seq_len, num_heads, head_dim, start_pos);
}
